...
typecheck: ## Run type checking with mypy
	uv run mypy gemini_file_search_tool

test: ## Run tests
	uv run pytest tests/

security-bandit: ## Run bandit security linter
	uv run bandit -r gemini_file_search_tool -c pyproject.toml
...
run: ## Run gemini-file-search-tool (usage: make run ARGS="...")
...
install-global: ## Install globally with uv tool
	uv tool uninstall gemini-file-search-tool || true
	uv build --force-pep517
	uv tool install .

uninstall-global: ## Uninstall global installation
	uv tool uninstall gemini-file-search-tool
